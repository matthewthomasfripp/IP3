...
const sizes: { [key: string]: { height: number, width: number, marginTop?: number, marginLeft?: number } } = {
...
export default function Logo(name: string) {
    return <div className='flex content-center h-5'>
        <Image
            src={`/logos/${name}.svg`}
            alt={name || 'Shop'}
            height={sizes[name].height}
            width={sizes[name].width}
            style={{ display: 'block', objectFit: 'contain', marginTop: sizes[name].marginTop || 0 }}
        />
    </div>
}